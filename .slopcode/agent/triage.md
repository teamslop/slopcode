...
- `slopcode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `slopcode web` view. **Do not** add for terminal, TUI, or general slopcode issues.
...
**Only** add if the issue mentions "zen" or "slopcode zen" or "slopcode black".

If the issue doesn't have "zen" or "slopcode black" in it then don't add zen label

#### core

Use for core server issues in `packages/slopcode/`, excluding `packages/slopcode/src/cli/cmd/tui/`.
...
TUI (`packages/slopcode/src/cli/cmd/tui/...`):
...
Core (`packages/slopcode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for slopcode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
