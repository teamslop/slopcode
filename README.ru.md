...
# Менеджеры пакетов
npm i -g slopcode-ai@latest        # или bun/pnpm/yarn
scoop install slopcode             # Windows
choco install slopcode             # Windows
brew install anomalyco/tap/slopcode # macOS и Linux (рекомендуем, всегда актуально)
...
nix run nixpkgs#slopcode           # или github:anomalyco/slopcode для самой свежей ветки dev
```
...
SlopCode также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/anomalyco/slopcode/releases) или с [slopcode.ai/download](https://slopcode.ai/download).
...
Подробнее об [agents](https://slopcode.ai/docs/agents).

### Документация

Больше информации о том, как настроить SlopCode: [**наши docs**](https://slopcode.ai/docs).
...
- 100% open source
- Не привязано к одному провайдеру. Мы рекомендуем модели из [SlopCode Zen](https://slopcode.ai/zen); но SlopCode можно использовать с Claude, OpenAI, Google или даже локальными моделями. По мере развития моделей разрыв будет сокращаться, а цены падать, поэтому важна независимость от провайдера.
