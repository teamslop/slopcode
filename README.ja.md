...
# パッケージマネージャー
npm i -g slopcode-ai@latest        # bun/pnpm/yarn でもOK
scoop install slopcode             # Windows
choco install slopcode             # Windows
brew install anomalyco/tap/slopcode # macOS と Linux（推奨。常に最新）
...
nix run nixpkgs#slopcode           # または github:anomalyco/slopcode で最新 dev ブランチ
```
...
SlopCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/slopcode/releases) から直接ダウンロードするか、[slopcode.ai/download](https://slopcode.ai/download) を利用してください。
...
[agents](https://slopcode.ai/docs/agents) の詳細はこちら。

### ドキュメント

SlopCode の設定については [**ドキュメント**](https://slopcode.ai/docs) を参照してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[SlopCode Zen](https://slopcode.ai/zen) で提供しているモデルを推奨しますが、SlopCode は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
