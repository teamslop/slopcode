...
`.slopcode/commands/test.md` oluştur:
...
slopcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

slopcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/slopcode/commands/`
- Proje başına: `.slopcode/commands/`
...
slopcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
