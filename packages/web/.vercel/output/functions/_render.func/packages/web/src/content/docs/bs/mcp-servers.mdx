...
Možete dodati vanjske alate u SlopCode koristeći _Model Context Protocol_, ili MCP. SlopCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [SlopCode Config](https://slopcode.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/slopcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
SlopCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, SlopCode će:
...
Ako server zahtijeva autentifikaciju, SlopCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `slopcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, SlopCode sigurno čuva tokene u `~/.local/share/slopcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u SlopCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz SlopCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete SlopCode sa vašim Sentry nalogom.
