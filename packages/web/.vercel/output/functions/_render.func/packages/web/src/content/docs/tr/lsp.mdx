...
slopcode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.
...
Slopcode bir dosyayı açtığında:
...
LSP sunucularını, slopcode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
