...
SlopCode Zen ist eine vom SlopCode-Team getestete und verifizierte Modellliste.

:::note
SlopCode Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in SlopCode.
Du meldest dich bei SlopCode Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit SlopCode funktionieren.
:::
...
SlopCode Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.
...
1. Melde dich bei **<a href={console}>SlopCode Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle SlopCode Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner SlopCode-Konfiguration hat das Format `slopcode/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `slopcode/gpt-5.2-codex`.
...
Wir haben SlopCode Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und SlopCode weiter mit anderen Providern nutzen kannst
