...
Kommandoen `slopcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en slopcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `slopcode.local` |
...
Indstil `SLOPCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `slopcode`, eller indstil `SLOPCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `slopcode serve` og `slopcode web`.
...
Når du kører `slopcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug slopcode-serveren til at interagere med slopcode programmatisk.
:::

Denne arkitektur lader slopcode understøtte flere klienter og lader dig interagere med slopcode programmatisk.

Du kan køre `slopcode serve` for at starte en selvstændig server. Hvis du har
slopcode TUI kørende, vil `slopcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af SlopCode [IDE](/docs/ide) plugins.
...
SlopCode-serveren viser følgende API'er.
