...
Per esempio, se la tua configurazione globale imposta `theme: "slopcode"` e `autoupdate: true`, e la configurazione del progetto imposta `model: "anthropic/claude-sonnet-4-5"`, la configurazione finale includera tutte e tre le impostazioni.
...
Metti la configurazione globale di SlopCode in `~/.config/slopcode/slopcode.json`. Usa la configurazione globale per preferenze valide per l'utente (ad es. temi, provider o keybind).
...
Puoi configurare il tema da usare in SlopCode tramite l'opzione `theme`.
