...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji slopcode.
...
te lub skonfiguruj własne za pomocą konfiguracji slopcode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.slopcode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `slopcode.json`:
...
- Globalnie: `~/.config/slopcode/modes/`
...
Jeśli nie ma wpływu na temperaturę, slopcode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna slopcode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.slopcode/modes/` dla trybów zapisanych dla projektu lub `~/.config/slopcode/modes/` dla trybów globalnych:
