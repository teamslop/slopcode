...
Create `.slopcode/commands/test.md`:
...
You can add custom commands through the SlopCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your SlopCode [config](/docs/config):
...
slopcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
