...
Le skill degli agenti permettono a SlopCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
SlopCode cerca in queste posizioni:

- Config di progetto: `.slopcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/slopcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, SlopCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.slopcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.slopcode/skills/git-release/SKILL.md` cosi':
...
SlopCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `slopcode.json`:
...
**Per agenti integrati** (in `slopcode.json`):
