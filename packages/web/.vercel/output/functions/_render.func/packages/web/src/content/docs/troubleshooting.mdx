...
- **macOS/Linux** (legacy): `~/.config/opencode/opencode.jsonc` (or `~/.config/opencode/opencode.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/opencode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\slopcode\slopcode.jsonc` (legacy path `%USERPROFILE%\.config\opencode\opencode.jsonc` is also supported)
...
- **Global plugins**
  - **macOS/Linux**: `~/.config/slopcode/plugins/` (legacy `~/.config/opencode/plugins/`)
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\slopcode\plugins` (legacy `%USERPROFILE%\.config\opencode\plugins`)
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.slopcode/plugins/` (legacy `<your-project>/.opencode/plugins/`)
...
If your `slopcode.json(c)` (or legacy `opencode.json(c)`) contains a `server` section, temporarily remove it and restart the desktop app.
