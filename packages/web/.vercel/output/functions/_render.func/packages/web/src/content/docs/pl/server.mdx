...
Komenda `slopcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient slopcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `slopcode.local` |
...
Ustaw `SLOPCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `slopcode` lub ustaw `SLOPCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `slopcode serve`, jak i `slopcode web`.
...
Kiedy uruchomisz `slopcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera slopcode do programowej interakcji z slopcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez slopcode i programową interakcję z kodem otwartym.

Możesz uruchomić `slopcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
slopcode TUI działa, `slopcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki slopcode [IDE](/docs/ide).
...
Serwer slopcode udostępnia następujące interfejsy API.
