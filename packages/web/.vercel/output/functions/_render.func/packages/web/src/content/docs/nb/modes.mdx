...
:::caution
Moduser er nå konfigurert gjennom alternativet `agent` i SlopCode-konfigurasjonen. De
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i SlopCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom SlopCode-konfigurasjonen.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.slopcode/plans/*.md` for å detaljere selve planen
...
Hvis ingen temperatur er spesifisert, bruker SlopCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
