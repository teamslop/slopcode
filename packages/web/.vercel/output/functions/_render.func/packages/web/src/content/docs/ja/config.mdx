...
たとえば、グローバル設定で `theme: "slopcode"` と `autoupdate: true` が設定され、プロジェクト設定で `model: "anthropic/claude-sonnet-4-5"` が設定されている場合、最終的な設定には 3 つの設定がすべて含まれます。
...
グローバル SlopCode 設定を `~/.config/slopcode/slopcode.json` に配置します。テーマ、プロバイダー、キーバインドなどのユーザー全体の設定にはグローバル設定を使用します。
...
設定ファイルには、[**`slopcode.dev/config.json`**](https://slopcode.dev/config.json) を使用します。
...
`small_model` オプションは、タイトル生成などの軽量タスク用に別のモデルを構成します。デフォルトでは、SlopCode は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`theme` オプションを使用して、SlopCode 設定で使用するテーマを構成できます。
