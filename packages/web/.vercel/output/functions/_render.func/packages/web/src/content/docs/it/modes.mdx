...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di slopcode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in slopcode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di slopcode.
...
slopcode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.slopcode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `slopcode.json`:
...
- Globale: `~/.config/slopcode/modes/`
- Progetto: `.slopcode/modes/`
...
Se non viene specificata alcuna temperatura, slopcode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di slopcode sia per quella specifica del progetto.
...
Crea file di modalita in `.slopcode/modes/` per modalita specifiche del progetto oppure in `~/.config/slopcode/modes/` per modalita globali:
