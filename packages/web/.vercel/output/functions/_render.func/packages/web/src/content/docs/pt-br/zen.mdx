...
:::note
O SlopCode Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no slopcode. Você faz login no SlopCode Zen e obtém sua chave de API. É **completamente opcional** e você não precisa usá-lo para utilizar o slopcode.
...
O SlopCode Zen é um gateway de IA que lhe dá acesso a esses modelos.
...
O [id do modelo](/docs/config/#models) na sua configuração do slopcode usa o formato `slopcode/<model-id>`. Por exemplo, para GPT 5.2 Codex, você usaria `slopcode/gpt-5.2-codex` na sua configuração.
...
- GLM 4.7 Free está disponível no slopcode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Kimi K2.5 Free está disponível no slopcode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- MiniMax M2.1 Free está disponível no slopcode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo oculto que está gratuito no slopcode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
...
4. Não ter **vinculação** permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o slopcode também.
