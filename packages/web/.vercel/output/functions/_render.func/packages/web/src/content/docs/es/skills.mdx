...
Las habilidades del agente permiten a SlopCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
SlopCode busca estas ubicaciones:

- Configuración del proyecto: `.slopcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/slopcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, SlopCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.slopcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.slopcode/skills/git-release/SKILL.md` así:
...
SlopCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `slopcode.json`:
...
**Para agentes integrados** (en `slopcode.json`):
