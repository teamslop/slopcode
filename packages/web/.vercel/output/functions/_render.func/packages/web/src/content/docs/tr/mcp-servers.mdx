...
_Model Bağlam Protokolü_ veya MCP kullanarak slopcode'a harici araçlar ekleyebilirsiniz. slopcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [slopcode Config](https://slopcode.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/slopcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
slopcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde slopcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa slopcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `slopcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından slopcode, belirteçleri `~/.local/share/slopcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra slopcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi slopcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve slopcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
