...
Crea `.slopcode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di SlopCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di SlopCode:
...
- Globale: `~/.config/slopcode/commands/`
- Per progetto: `.slopcode/commands/`
...
slopcode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
