...
Du kan legge til eksterne verktøy i SlopCode ved å bruke _Model Context Protocol_ eller MCP. SlopCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [SlopCode Config](https://slopcode.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/slopcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
SlopCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil SlopCode:
...
Hvis serveren krever autentisering, vil SlopCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `slopcode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil SlopCode lagre tokens sikkert i `~/.local/share/slopcode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i SlopCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom SlopCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble SlopCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver SlopCode-samtale oppført som en separat økt.
