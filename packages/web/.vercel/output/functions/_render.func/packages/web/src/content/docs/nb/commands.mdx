...
Opprett `.slopcode/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom SlopCode-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i SlopCode [config](/docs/config):
...
- Per prosjekt: `.slopcode/commands/`
...
SlopCode inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
