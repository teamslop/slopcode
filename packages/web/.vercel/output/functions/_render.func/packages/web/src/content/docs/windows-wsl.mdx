---
title: Windows (WSL)
description: Run SlopCode on Windows using WSL for the best experience.
---

import { Steps } from "@astrojs/starlight/components"

While SlopCode can run directly on Windows, we recommend using [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install) for the best experience. WSL provides a Linux environment that works seamlessly with SlopCode's features.

:::tip[Why WSL?]
WSL offers better file system performance, full terminal support, and compatibility with development tools that SlopCode relies on.
:::
...
2. **Install SlopCode in WSL**
   ...
   Once WSL is set up, open your WSL terminal and install SlopCode using one of the [installation methods](/docs/).
...
:::caution
When using `--hostname 0.0.0.0`, set `SLOPCODE_SERVER_PASSWORD` to secure the server.
:::
...
:::tip
For the smoothest experience, consider cloning/copying your repo into the WSL filesystem (for example under `~/code/`) and running SlopCode there.
:::
...
- Use VS Code's [WSL extension](https://code.visualstudio.com/docs/remote/wsl) alongside SlopCode for an integrated development workflow
