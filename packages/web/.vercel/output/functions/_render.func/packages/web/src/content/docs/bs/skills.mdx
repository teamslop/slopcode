...
Agent skills omogucavaju SlopCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
SlopCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.slopcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/slopcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, SlopCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.slopcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.slopcode/skills/git-release/SKILL.md` ovako:
...
SlopCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `slopcode.json`:
...
**Za ugradene agente** (u `slopcode.json`):
