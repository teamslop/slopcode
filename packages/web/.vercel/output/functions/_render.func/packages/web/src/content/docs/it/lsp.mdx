...
SlopCode si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di slopcode.
