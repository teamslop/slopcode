...
:::caution
Modes are now configured through the `agent` option in the slopcode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in slopcode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the slopcode config.
...
slopcode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.slopcode/plans/*.md` to detail the plan itself
...
Configure modes in your `slopcode.json` config file:
...
both the global slopcode config and the project specific config.
...
Create mode files in `.slopcode/modes/` for project-specific modes or `~/.config/slopcode/modes/` for global modes:
