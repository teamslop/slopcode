---
title: Plugins
description: Écrivez vos propres plugins pour étendre SlopCode.
---

Les plugins vous permettent d'étendre SlopCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de SlopCode.
...
- `.slopcode/plugins/` - Plugins au niveau du projet
- `~/.config/slopcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/slopcode/node_modules/`.
...
1. Configuration globale (`~/.config/slopcode/slopcode.json`)
2. Configuration du projet (`slopcode.json`)
3. Répertoire global des plugins (`~/.config/slopcode/plugins/`)
4. Répertoire des plugins du projet (`.slopcode/plugins/`)
...
SlopCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK slopcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre slopcode.
...
:::note
Si vous utilisez l'application de bureau SlopCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez slopcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à slopcode :
...
L'assistant `tool` crée un outil personnalisé que slopcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour slopcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://slopcode.dev/docs/sdk) pour plus de détails.
