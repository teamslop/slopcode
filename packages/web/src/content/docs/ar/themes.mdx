...
مع SlopCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم SlopCode سمتنا `slopcode`.
...
يأتي SlopCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق SlopCode مظهر terminal لديهم
...
يمكنك اختيار سمة بفتح منتقي السمات باستخدام الأمر `/themes`. أو يمكنك تحديدها في [الضبط](/docs/config).
...
يدعم SlopCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/slopcode/themes/*.json` أو `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.slopcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.slopcode/themes/*.json`
