...
يشغّل الأمر `slopcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل slopcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `slopcode.local` |
...
عيّن `SLOPCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `slopcode`، أو عيّن `SLOPCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `slopcode serve` و `slopcode web`.
...
عند تشغيل `slopcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم slopcode للتفاعل مع slopcode برمجيا.
:::

تتيح هذه البنية لـ slopcode دعم عدة عملاء وتمكّنك من التفاعل مع slopcode برمجيا.

يمكنك تشغيل `slopcode serve` لبدء خادم مستقل. إذا كانت واجهة slopcode في terminal (TUI)
قيد التشغيل، فسيبدأ `slopcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات SlopCode لـ [IDE](/docs/ide).
...
يعرض خادم slopcode واجهات API التالية.
