...
أنشئ الملف `.slopcode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات SlopCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ SlopCode:
...
- على مستوى النظام: `~/.config/slopcode/commands/`
- لكل مشروع: `.slopcode/commands/`
...
يتضمن slopcode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
