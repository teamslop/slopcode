---
title: 规则
description: 为 slopcode 设置自定义指令。
---
...
- **全局规则**：`~/.claude/CLAUDE.md`（在没有 `~/.config/slopcode/AGENTS.md` 的情况下使用）
...
2. **全局文件**，位于 `~/.config/slopcode/AGENTS.md`
...
:::tip
对于 monorepo 或具有共享标准的项目，使用 `slopcode.json` 配合 glob 模式（如 `packages/*/AGENTS.md`）比手动指定指令更易于维护。
:::
