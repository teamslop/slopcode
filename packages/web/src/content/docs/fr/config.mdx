...
Par exemple, si votre configuration globale définit `theme: "slopcode"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
Placez votre configuration globale SlopCode dans `~/.config/slopcode/slopcode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration SlopCode via l'option `theme`.
