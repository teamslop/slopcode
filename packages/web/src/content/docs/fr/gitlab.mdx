---
title: GitLab
description: Utilisez SlopCode dans les issues GitLab et les merge requests.
---
...
SlopCode fonctionne dans un pipeline GitLab régulier. Vous pouvez l'intégrer dans un pipeline en tant que [composant CI](https://docs.gitlab.com/ee/ci/components/)

Ici, nous utilisons un composant CI/CD créé par la communauté pour SlopCode — [nagyv/gitlab-slopcode](https://gitlab.com/nagyv/gitlab-slopcode).

---

### Fonctionnalités

- **Utiliser une configuration personnalisée par tâche** : configurez SlopCode avec un répertoire de configuration personnalisé, par exemple `./config/#custom-directory` pour activer ou désactiver la fonctionnalité pour chaque appel d'SlopCode.
- **Configuration minimale** : le composant CI configure SlopCode en arrière-plan, il vous suffit de créer la configuration SlopCode et le prompt initial.
...
1. Stockez votre authentification SlopCode JSON en tant que variables d'environnement CI de type fichier sous **Paramètres** > **CI/CD** > **Variables**. Assurez-vous de les marquer comme « Masquées et protégées ».
...
Pour plus d'entrées et de cas d'utilisation [consultez la doc](https://gitlab.com/explore/catalog/nagyv/gitlab-slopcode) pour ce composant.
...
- **Correction et implémentation** : demandez à SlopCode de résoudre une issue ou d'implémenter une fonctionnalité.
...
:::tip
Vous pouvez configurer pour utiliser une phrase de déclenchement différente de `@slopcode`.
:::
...
SlopCode lira l'issue et répondra avec une explication claire.
...
SlopCode créera une nouvelle branche, mettra en œuvre les modifications et ouvrira une merge request avec les modifications.
...
SlopCode examinera la merge request et fournira des commentaires.
