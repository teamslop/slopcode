...
1. TUI에서 `/connect` 명령을 실행하고 `slopcode`를 선택한 뒤, [slopcode.dev/auth](https://slopcode.dev/auth)로 이동합니다.
...
[slopcode-gitlab-auth](https://www.npmjs.com/package/@gitlab/slopcode-gitlab-auth) 홈페이지에 추가 문서가 있습니다.
...
GitHub Copilot 구독을 사용하여 slopcode:
...
이벤트에서 slopcode를 통해 자동으로 구성되지 않는 Helicone의 기능 또는 모델을 볼 수 있습니다.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) 기능으로 그룹 관련 LLM 요청이 가능합니다. [slopcode-helicone-session](https://github.com/H2Shami/slopcode-helicone-session) 플러그인을 사용하여 각 slopcode 대화를 Helicone 세션으로 자동 로그인하십시오.
...
플러그인은 `Helicone-Session-Id` 및 `Helicone-Session-Name` 헤더를 귀하의 요청에 주사합니다. Helicone의 세션 페이지에서는 별도의 세션으로 나열된 각 slopcode 대화를 볼 수 있습니다.
...
:::tip
Ollama는 slopcode를 자동으로 구성할 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/slopcode)를 참조하십시오.
:::
...
3. slopcode에서 사용을 위한 API 열쇠를 복사하십시오.
...
6. ** 중요 **: slopcode의 클라우드 모델을 사용하기 전에, 로컬 모델 정보를 끌어야 합니다:
...
1. 로그인 **<a href={console}>SlopCode Zen</a>** and click**Create API Key**.

2. `/connect` 명령을 실행하고 **SlopCode Zen**를 검색하십시오.
...
slopcode config를 통해 추가 모델을 추가할 수 있습니다.
...
5. 당신은 또한 당신의 slopcode config를 통해 그들을 주문을 받아서 만들 수 있습니다. 공급자 지정의 예입니다.
...
slopcode를 사용하여 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)를 사용하려면:
...
slopcode config를 통해 모델을 사용자 정의 할 수 있습니다. 공급자 routing 순서를 지정하는 예입니다.
...
:::tip
slopcode를 사용하여 OpenAI 호환 공급자를 사용할 수 있습니다. 가장 현대적인 AI 제공 업체는 OpenAI 호환 API를 제공합니다.
:::
...
`limit` 필드를 사용하면 slopcode가 얼마나 많은 컨텍스트를 이해 할 수 있습니다. 표준 공급자는 model.dev에서 자동적으로 당깁니다.
...
1. **주의 설정 확인 **: `slopcode auth list`를 실행하여 자격 증명을 볼 수 있습니다.
   공급자는 config에 추가됩니다.

이것은 Amazon Bedrock과 같은 공급자에 적용되지 않습니다. 환경 변수에 의존합니다.

2. 주문 공급자를 위해, slopcode config를 검사하고:

- `/connect` 명령에 사용되는 공급자 ID가 slopcode config에서 ID를 일치시킵니다.
