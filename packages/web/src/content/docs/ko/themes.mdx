...
- slopcode가 terminal의 외관과 일치해야 합니다.
...
테마를 `/themes` 명령어로 선택하여 테마를 선택할 수 있습니다. 또는 [config](/docs/config)에서 지정할 수 있습니다.
...
slopcode는 사용자가 쉽게 테마를 만들 수 있도록 유연한 JSON 기반 테마 시스템을 지원합니다.
...
1.**Built-in themes** - 이것은 바이너리에 내장되어 있습니다. 2. **사용자 설정 디렉토리 ** - `~/.config/slopcode/themes/*.json` 또는 `$XDG_CONFIG_HOME/slopcode/themes/*.json`에서 정의 3. ** 루트 디렉토리 ** - `<project-root>/.slopcode/themes/*.json`에서 정의 4. **현재 작업 디렉토리 ** - `./.slopcode/themes/*.json`에서 정의
