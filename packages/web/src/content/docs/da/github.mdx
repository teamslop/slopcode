---
title: GitHub
description: Brug SlopCode i GitHub-problemer og Pull Requests.
---

SlopCode integreres med din GitHub arbejdsgang. Nævn `/slopcode` eller `/oc` i din kommentar, og SlopCode vil udføre opgaver i din GitHub Actions-løber.
...
1. **Installationsprogrammet GitHub-appen**
   ...
   Gå over til [**github.com/apps/slopcode-agent**](https://github.com/apps/slopcode-agent). Sørg for, at det er installeret på mållageret.

2. **Tilføj arbejdsgangen**

   Tilføj følgende workflow-fil til `.github/workflows/slopcode.yml` i din repo. Sørg for at indstille de relevante `model` og nødvendige API-nøgler i `env`.
...
- `model`: Den model, der skal bruges med SlopCode. Antager formatet `provider/model`. Dette er **påkrævet**.
...
- `token`: Valgfrit GitHub adgangstoken til at udføre operationer såsom oprettelse af kommentarer, begå ændringer og åbning af Pull Requests. Som standard bruger SlopCode installationsadgangstokenet fra SlopCode GitHub-appen, så commits, kommentarer og Pull Requests ser ud til at komme fra appen.

  Alternativt kan du bruge GitHub Action runners [built-in `GITHUB_TOKEN`](SlopCode) uden at installere SlopCode GitHub appen. Bare sørg for at give de nødvendige tilladelser i dit workflow:
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentarer og problemer eller PR       | Nævn `/slopcode` eller `/oc` i din kommentar. SlopCode læser kontekst og kan oprette brancher, åbne PR'er eller svare.  |
| `pull_request_review_comment` | Kommenter specifikke kodelinjer i en PR | Nævn `/slopcode` eller `/oc`, mens du gennemgår koden. SlopCode modtager filsti, linjenumre og diff-kontekst.           |
...
For `pull_request` hændelser, hvis der ikke er angivet nogen `prompt`, vil SlopCode som standard gennemgå Pull Requesten.
...
- **Forklar et problem**
  ...
  SlopCode vil læse hele tråden, inklusive alle kommentarer, og svare med en klar forklaring.
...
- **Løs et problem**
  ...
  Og SlopCode vil oprette en ny branch, implementere ændringer og åbne en PR med ændringer.
...
- **Gennemgå PR'er og foretag ændringer**
  ...
  SlopCode vil implementere den ønskede ændring og forpligtelse til den samme PR.
...
- **Gennemgå specifikke kodelinjer**
  ...
  Når du kommenterer på specifikke linjer, modtager SlopCode:
