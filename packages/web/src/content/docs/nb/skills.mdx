...
Agentferdigheter lar SlopCode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
slopcode søker etter disse stedene:

- Prosjektkonfigurasjon: `.slopcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/slopcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går slopcode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.slopcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.slopcode/skills/git-release/SKILL.md` slik:
...
slopcode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `slopcode.json`:
...
**For innebygde agenter** (i `slopcode.json`):
