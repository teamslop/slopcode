...
I plugin ti permettono di estendere SlopCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di SlopCode.
...
:::note
Se usi l'app desktop di SlopCode, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
L'helper `tool` crea uno strumento personalizzato che slopcode puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
