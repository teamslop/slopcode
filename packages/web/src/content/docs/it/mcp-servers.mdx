...
Puoi aggiungere strumenti esterni a SlopCode usando il _Model Context Protocol_, o MCP. SlopCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [SlopCode Config](https://slopcode.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/slopcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
SlopCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, SlopCode:
...
Se il server richiede autenticazione, SlopCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `slopcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, SlopCode memorizzera' i token in modo sicuro in `~/.local/share/slopcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in SlopCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione SlopCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare SlopCode al tuo account Sentry.
