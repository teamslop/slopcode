---
title: GitLab
description: Use o slopcode em problemas e solicitações de mesclagem do GitLab.
---

O slopcode se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o slopcode será executado em seus runners do GitLab.

---

## GitLab CI

O slopcode funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para slopcode — [nagyv/gitlab-slopcode](https://gitlab.com/nagyv/gitlab-slopcode).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o slopcode com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do slopcode.
- **Configuração mínima**: O componente CI configura o slopcode em segundo plano, você só precisa criar a configuração do slopcode e o prompt inicial.
...
1. Armazene seu JSON de autenticação do slopcode como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-slopcode/slopcode@2
       inputs:
         ...
         auth_json: $SLOPCODE_AUTH_JSON # O nome da variável para seu JSON de autenticação do SlopCode
         command: optional-custom-command
         message: "Seu prompt aqui"
   ```

Para mais entradas e casos de uso [consulte a documentação](https://gitlab.com/explore/catalog/nagyv/gitlab-slopcode) deste componente.

---

## GitLab Duo

O slopcode se integra ao seu fluxo de trabalho do GitLab.
Mencione `@slopcode` em um comentário, e o slopcode executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao slopcode para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O slopcode é executado em seus runners do GitLab.

---

### Configuração

O slopcode é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do SlopCode"
...
Aqui estão alguns exemplos de como você pode usar o slopcode no GitLab.

:::tip
Você pode configurar para usar uma frase de gatilho diferente de `@slopcode`.
:::
...
- **Explicar um problema**
  ...
  O slopcode lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O slopcode criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O slopcode revisará a solicitação de mesclagem e fornecerá feedback.
