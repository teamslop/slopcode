...
![slopcode Web - Nova Sessão](../../../assets/web/web-homepage-new-session.png)
...
:::caution
Se `SLOPCODE_SERVER_PASSWORD` não estiver definido, o servidor ficará sem segurança. Isso é aceitável para uso local, mas deve ser configurado para acesso à rede.
:::

:::tip[Usuários do Windows]
Para a melhor experiência, execute `slopcode web` a partir do [WSL](/docs/windows-wsl) em vez do PowerShell. Isso garante acesso adequado ao sistema de arquivos e integração com o terminal.
:::
...
![slopcode Web - Sessão Ativa](../../../assets/web/web-homepage-active-session.png)
