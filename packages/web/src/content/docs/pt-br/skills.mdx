...
As habilidades do agente permitem que o slopcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O slopcode pesquisa nesses locais:

- Configuração do projeto: `.slopcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/slopcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o slopcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.slopcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.slopcode/skills/git-release/SKILL.md` assim:
...
O slopcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `slopcode.json`:
...
**Para agentes embutidos** (em `slopcode.json`):
