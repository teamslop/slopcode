...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do slopcode. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no slopcode permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do slopcode.
...
O slopcode vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.slopcode/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `slopcode.json`:
...
- Global: `~/.config/slopcode/modes/`
- Projeto: `.slopcode/modes/`

```markdown title="~/.config/slopcode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o slopcode usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do slopcode quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.slopcode/modes/` para modos específicos do projeto ou `~/.config/slopcode/modes/` para modos globais:

```markdown title=".slopcode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/slopcode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
