...
Создайте `.slopcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию slopcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем slopcode [config](/docs/config):
...
- Глобальный: `~/.config/slopcode/commands/`
- Для каждого проекта: `.slopcode/commands/`
...
slopcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
