...
Erstellen Sie `.slopcode/commands/test.md`:

```md title=".slopcode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die SlopCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem SlopCode [config](/docs/config):

```json title="slopcode.jsonc" {4-12}
{
...
- Global: `~/.config/slopcode/commands/`
- Pro Projekt: `.slopcode/commands/`

```markdown title="~/.config/slopcode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".slopcode/commands/component.md"
---
...
```md title=".slopcode/commands/create-file.md"
---
...
```md title=".slopcode/commands/analyze-coverage.md"
---
...
```md title=".slopcode/commands/review-changes.md"
---
...
```md title=".slopcode/commands/review-component.md"
---
...
SlopCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
