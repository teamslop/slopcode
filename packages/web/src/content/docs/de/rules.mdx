---
title: Regeln
description: Lege eigene Anweisungen fuer slopcode fest.
---

Du kannst slopcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in slopcode den Befehl `/init` aus.
...
So kann slopcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht slopcode sie zu erweitern.
...
slopcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/slopcode/AGENTS.md` speichern.
Diese gelten in allen slopcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt SlopCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/slopcode/AGENTS.md` existiert)
...
Beim Start sucht slopcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/slopcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `slopcode.json` oder in `~/.config/slopcode/slopcode.json` angeben.
...
slopcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit slopcode.json

Empfohlen ist das `instructions`-Feld in der `slopcode.json`:
...
Du kannst slopcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `slopcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
