---
title: Windows (WSL)
description: Nutze SlopCode unter Windows am besten mit WSL.
---

import { Steps } from "@astrojs/starlight/components"

SlopCode laeuft zwar direkt unter Windows, die beste Erfahrung bekommst du aber mit [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install).
WSL bietet eine Linux-Umgebung, die mit den SlopCode-Features nahtlos funktioniert.

:::tip[Warum WSL?]
WSL bietet bessere Dateisystem-Performance, vollstaendige Terminal-Unterstuetzung und hohe Kompatibilitaet mit den Entwicklungswerkzeugen, die SlopCode nutzt.
:::
...
2. **Install SlopCode in WSL**
   ...
   Wenn WSL eingerichtet ist, oeffne dein WSL-Terminal und installiere SlopCode mit einer der [Installationsmethoden](/docs/).
...
:::caution
Wenn du `--hostname 0.0.0.0` verwendest, setze `SLOPCODE_SERVER_PASSWORD`, um den Server zu schuetzen.
...
Wenn du `slopcode web` aus WSL startest, bleiben Dateizugriff und Terminal-Integration sauber.
...
:::tip
Fuer die beste Performance kannst du dein Repo direkt in das WSL-Dateisystem klonen/kopieren (z. B. nach `~/code/`) und SlopCode dort starten.
:::
