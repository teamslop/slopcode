---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um SlopCode zu erweitern.
---

Mit Plugins können Sie SlopCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von SlopCode zu ändern.
...
- `.slopcode/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/slopcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/slopcode/slopcode.json`)
2. Projektkonfiguration (`slopcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/slopcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.slopcode/plugins/`)
...
SlopCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein SlopCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie SlopCode erweitern können.
...
:::note
Wenn Sie die SlopCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass SlopCode `.env`-Dateien liest:
...
Plugins können SlopCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das SlopCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für SlopCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://slopcode.dev/docs/sdk).
