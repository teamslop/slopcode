...
Możesz dodać dodatkowe informacje do slopcode za pomocą _Model Context Protocol_, w skrócie MCP. slopcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [slopcode Config](https://slopcode.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/slopcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
slopcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, slopcode:
...
Jeśli serwer wymaga uwierzytelnienia, slopcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `slopcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji slopcode bezpieczne przechowa tokeny w `~/.local/share/slopcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w slopcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie slopcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć slopcode z kontem Sentry.
