...
:::caution
Modlar artık slopcode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

slopcode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya slopcode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
slopcode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.slopcode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`slopcode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, slopcode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global slopcode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.slopcode/modes/` veya genel modlar için `~/.config/slopcode/modes/`'de mod dosyaları oluşturun:
