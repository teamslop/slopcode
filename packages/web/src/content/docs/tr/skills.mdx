...
Ajan becerileri, slopcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
slopcode su konumlari tarar:

- Proje konfigurasyonu: `.slopcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/slopcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda slopcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.slopcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.slopcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
slopcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`slopcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`slopcode.json` icinde):
