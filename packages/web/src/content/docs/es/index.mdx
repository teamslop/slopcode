---
title: Introducción
description: Comience con SlopCode.
---
...
Para obtener la mejor experiencia en Windows, recomendamos utilizar [Windows Subsystem for Linux (WSL)](/docs/windows-wsl). Proporciona un mejor rendimiento y compatibilidad total con las funciones de SlopCode.
:::
...
También puede obtener el binario de [Versiones](https://github.com/anomalyco/slopcode/releases).
...
Si es nuevo en el uso de proveedores LLM, le recomendamos usar [SlopCode Zen](/docs/zen).
Es una lista seleccionada de modelos que han sido probados y verificados por el equipo de SlopCode.
...
Esto hará que SlopCode analice su proyecto y cree un archivo `AGENTS.md` en
...
Esto ayuda a SlopCode a comprender la estructura del proyecto y los patrones de codificación.
usado.
...
Ahora está listo para usar SlopCode para trabajar en su proyecto. No dudes en preguntarle
¡cualquier cosa!
...
1. **Crea un plan**
   ...
   SlopCode tiene un _Modo Plan_ que desactiva su capacidad para realizar cambios y
   ...
   Quiere darle a SlopCode suficientes detalles para entender lo que quiere. ayuda
   hablar con él como si estuviera hablando con un desarrollador junior de su equipo.

   :::tip
   Dale a SlopCode mucho contexto y ejemplos para ayudarlo a comprender lo que
   desear.
   :::
...
2. **Repetir el plan**
   ...
   SlopCode puede escanear cualquier imagen que le proporcione y agregarla al mensaje. Puede
...
Para cambios más sencillos, puede pedirle a SlopCode que lo construya directamente.
...
Desea asegurarse de proporcionar una buena cantidad de detalles para que SlopCode tome la decisión correcta.
cambios.
...
SlopCode ahora revertirá los cambios que realizó y mostrará su mensaje original
de nuevo.
...
Las conversaciones que tengas con SlopCode pueden ser [compartidas con tu
equipo](/docs/share).
...
Aquí hay una [conversación de ejemplo](https://slopcode.ai/s/4XP1fce5) con SlopCode.
...
Para personalizarlo, recomendamos [elegir un tema](/docs/themes), [personalizar las combinaciones de teclas](/docs/keybinds), [configurar formateadores de código](/docs/formatters), [crear comandos personalizados](/docs/commands) o jugar con la [configuración SlopCode](/docs/config).
