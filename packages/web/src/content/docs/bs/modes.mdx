...
ove ili konfigurirajte svoje putem slopcode config.
...
slopcode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.slopcode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `slopcode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/slopcode/modes/`
- Projekat: `.slopcode/modes/`
...
Ako temperatura nije navedena, slopcode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.slopcode/modes/` za specifične načine rada ili `~/.config/slopcode/modes/` za globalne načine:
