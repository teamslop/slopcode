...
Na primjer, ako vaša globalna konfiguracija postavlja `theme: "slopcode"` i `autoupdate: true`, a vaša projektna konfiguracija postavlja `model: "anthropic/claude-sonnet-4-5"`, konačna konfiguracija će uključivati ​​sve tri postavke.
...
2. **Globalna konfiguracija** (`~/.config/slopcode/slopcode.json`) - korisničke postavke
...
Postavite svoju globalnu SlopCode konfiguraciju u `~/.config/slopcode/slopcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su teme, provajderi ili veze tipki.
...
Možete konfigurirati temu koju želite koristiti u svojoj SlopCode konfiguraciji putem opcije `theme`.
