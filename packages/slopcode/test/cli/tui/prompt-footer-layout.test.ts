...
const box = (id: string, key: string, labels: string[], required = false, compact = 0, drop = 0): PromptFooterItem => ({
  id,
  required,
  compact,
  drop,
...
const group = (id: string, left: string, right: string, labels: string[], required = false, compact = 0, drop = 0): PromptFooterItem => ({
  id,
  required,
  compact,
  drop,
...
const status = (labels: string[], compact = 100, drop = 10): PromptFooterItem => ({
  id: "status",
  compact,
  drop,
  variants: labels.map((text) => ({ parts: [{ text, tone: "muted" as const }] })),
})

const live = [
  status(["still running... 9m 44s", "running 9m 44s", "9m 44s"]),
  box("interrupt", "esc", ["interrupt", "stop"], true, 20, 0),
  box("variant", "ctrl+t", ["variants", "var"], false, 40, 50),
]
...
describe("prompt footer layout", () => {
  ...
  test("compacts status text before touching required controls", () => {
    const result = layoutPromptFooter({ width: 24, items: live })

    expect(result.items.map((item) => item.id)).toEqual(["status", "interrupt"])
    expect(result.items[0]?.variant).toBeGreaterThan(0)
    expect(result.items[1]?.variant).toBe(0)
  })

  test("drops status before required interrupt on extremely narrow widths", () => {
    const result = layoutPromptFooter({ width: 8, items: live })
