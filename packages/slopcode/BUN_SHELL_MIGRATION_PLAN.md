...
Replace runtime Bun shell template-tag usage in `packages/slopcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/slopcode/src` is removed except:
