...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for slopcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal slopcode sessions
...
slopcode acp --cwd /path/to/project
```
...
```bash
SLOPCODE_ENABLE_QUESTION_TOOL=1 slopcode acp
```
...
- **Terminal Integration**: Full terminal support via slopcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | slopcode acp
```
...
ACP sessions map cleanly to slopcode's internal session model:
