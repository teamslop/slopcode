...
export namespace DaemonRegistry {
  ...
  function key(directory: string) {
    return createHash("sha1").update(normalize(directory)).digest("hex")
  }
  ...
  export async function read(directory: string) {
    const json = await Bun.file(paths(directory).info)
      .json()
      .catch(() => undefined)
    if (!json) return
  ...
  export async function write(info: Info) {
    const file = paths(info.directory).info
  ...
  export async function remove(directory: string) {
    const { info, lock } = paths(directory)
  ...
  export async function acquire(directory: string, timeout = 10_000) {
    const lock = paths(directory).lock
