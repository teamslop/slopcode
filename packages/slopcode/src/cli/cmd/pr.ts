      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for slopcode session link in PR body
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await $`slopcode import ${sessionUrl}`.nothrow()
        ...
        // Launch slopcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const slopcodeArgs = sessionId ? ["-s", sessionId] : []
        const slopcodeProcess = spawn("slopcode", slopcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          slopcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`slopcode exited with code ${code}`))
          })
          slopcodeProcess.on("error", reject)
        })
      },
    })
  },
