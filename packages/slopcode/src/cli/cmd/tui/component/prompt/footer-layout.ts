...
function compact(item: PromptFooterItem) {
  return item.compact ?? 0
}

function drop(item: PromptFooterItem) {
  return item.drop ?? 0
}

function remove(items: PromptFooterItem[], count: number) {
  if (count >= items.length) return items
  const cut = items.length - count
  const gone = new Set(
    items
      .map((item, index) => ({ item, index }))
      .filter((entry) => !entry.item.required)
      .sort((a, b) => drop(b.item) - drop(a.item) || b.index - a.index)
      .slice(0, cut)
      .map((entry) => entry.index),
  )
  return items.filter((_, index) => !gone.has(index))
}

function candidate(items: PromptFooterItem[], steps: number[]) {
  return items.reduce((best, item, index) => {
    if (steps[index]! >= item.variants.length - 1) return best
    if (!best) return { item, index }
    if (compact(item) !== compact(best.item)) return compact(item) > compact(best.item) ? { item, index } : best
    return index > best.index ? { item, index } : best
  }, undefined as { item: PromptFooterItem, index: number } | undefined)
}
...
function fit(items: PromptFooterItem[], pad: number, gap: number, width: number) {
  ...
  for (let count = items.length; count >= min; count--) {
    const shown = remove(items, count)
    const steps = shown.map(() => 0)
    if (used(shown, steps, pad, gap) <= width) return build(shown, steps, pad, gap, width)
    while (true) {
      const next = candidate(shown, steps)
      if (!next) break
      steps[next.index] = steps[next.index]! + 1
      if (used(shown, steps, pad, gap) <= width) return build(shown, steps, pad, gap, width)
    }
  }
