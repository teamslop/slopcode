...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("slopcode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("slopcode_lib=debug,slopcode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("slopcode_lib=info,slopcode_desktop=info,sidecar=info")
        }
    });
