...
const CLI_INSTALL_DIR: &str = ".slopcode/bin";
const CLI_BINARY_NAME: &str = "slopcode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("slopcode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "SLOPCODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "SLOPCODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("SLOPCODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut env_prefix = vec![
                "SLOPCODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "SLOPCODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "SLOPCODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "SLOPCODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "SLOPCODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "SLOPCODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("SLOPCODE_SERVER_USERNAME", "slopcode".to_string()),
        ("SLOPCODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn slopcode");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("SLOPCODE_CLIENT"), Some(&"desktop".to_string()));
    }
